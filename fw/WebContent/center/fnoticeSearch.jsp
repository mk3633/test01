...
// 검색어를 포함하는 게시판 글개수
int count = bdao.getBoardCount2(search);
...
if(count!=0){
	boardList2=bdao.getBoardList2(startRow,pageSize, search);
}
// 날짜 형태 -(대쉬) 에서 . 으로 교체
...
<!-- 게시판 -->
<article>
<h1 class="subtitle"><em>D</em>ownload       <span>[전체 글개수 : <%=count %> ]</span></h1>
<table id="notice">
...
<tr><td><%=bb.getNum() %></td>
	<td class="left">
	<a href="fcontent.jsp?num=<%=bb.getNum()%>&pageNum=<%=pageNum%>"><%=bb.getSubject() %></a>
	</td>
    <td><%=bb.getName() %></td>
<%--     <td><%=bb.getDate() %></td> --%>
